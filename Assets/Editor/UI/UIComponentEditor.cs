...
{
    /// <summary> 自定义UIComponent类在界面中的显示与功能 </summary>
    [CustomEditor(typeof(UIComponent), true)]
    public class UIComponentEditor : Editor
    {
        private string ReferenceData_key = "key";
        private string ReferenceData_gameObject = "gameObject";
        private string ReferenceManager_data = "data";
        ...
        private string searchKey
        {
            ...
            set
            {
                ...
                {
                    _searchKey = value;
                    heroPrefab = rManager.Get<Object>(searchKey);
                }
            }
        }
        private UIComponent rManager { get; set; }
        ...
        {
            //使UIComponent支持撤销操作，还有Redo，不过没有在这里使用
            Undo.RecordObject(rManager, "Changed Settings");
            var dataProperty = serializedObject.FindProperty(ReferenceManager_data);//按名称查找序列化属性。
            return dataProperty;
        }
        ...
        private void OnUIContent2(SerializedProperty dataProperty)
        {
            ...
            for (int i = rManager.data.Count - 1; i >= 0; i--)
            {
                EditorGUILayout.BeginHorizontal();
                property = dataProperty.GetArrayElementAtIndex(i).FindPropertyRelative(ReferenceData_key);
                property.stringValue = EditorGUILayout.TextField(property.stringValue, GUILayout.Width(200));
                property = dataProperty.GetArrayElementAtIndex(i).FindPropertyRelative(ReferenceData_gameObject);
        ...
        private void AddReference(SerializedProperty dataProperty, string key, Object obj)
        {
            ...
            element.FindPropertyRelative(ReferenceData_key).stringValue = key;
            element.FindPropertyRelative(ReferenceData_gameObject).objectReferenceValue = obj;
        }
        ...
        private void ReplaceReferenceName(SerializedProperty dataProperty, string key)
        {
            ...
            for (int i = dataProperty.arraySize - 1; i >= 0; i--)
            {
                ...
                var keyProperty = serializedProperty.FindPropertyRelative(ReferenceData_key);//物体名称
                var gameObjectProperty = serializedProperty.FindPropertyRelative(ReferenceData_gameObject);//查找物体
            ...
            EditorUtility.SetDirty(rManager);
            OnUIUpdata();
        }
        ...
        {
            var dataProperty = serializedObject.FindProperty(ReferenceManager_data);
            for (int i = dataProperty.arraySize - 1; i >= 0; i--)
            {
                var gameObjectProperty = dataProperty.GetArrayElementAtIndex(i).FindPropertyRelative(ReferenceData_gameObject);//查找物体
                if (gameObjectProperty.objectReferenceValue == null)
                {
                    dataProperty.DeleteArrayElementAtIndex(i);
                    EditorUtility.SetDirty(rManager);
        ...
        private void OoGetKeyGos(SerializedProperty dataProperty)
        {
            ...
            ACLoopGetKeywordGO(rManager.gameObject.transform, Prefix, ref gameObjects);
        ...
        private void OnCreatCommonGetCode(SerializedProperty dataProperty, string eTUITool_ClassName)
        {
            ...
            for (int i = dataProperty.arraySize - 1; i >= 0; i--)
            {
                ...
                {
                    //T_Close.GetComponent<Button>().onClick.AddListener(OnT_Close);
                    Component component = (gameObjectProperty.objectReferenceValue as GameObject).GetComponent(type);
                    if (component != null)
                        sb.AppendLine($"{gameObjectProperty.objectReferenceValue.name}.GetComponent<{eTUITool_ClassName}>().onClick.AddListener(On{gameObjectProperty.objectReferenceValue.name});");
                }
            ...
            for (int i = dataProperty.arraySize - 1; i >= 0; i--)
            {
                ...
                if (type == typeof(Button))
                {
                    ...
                    if (component != null)
                    {
                        ...
                        sb.AppendLine($"public static void On{gameObjectProperty.objectReferenceValue.name}({ETUITool_ClassName1})\t\n{{\t\n}}");
                    }
                }
            }
            Debug.Log(sb.ToString());
